...
export const ProvinceForm: React.FC<ProvinceFormProps> = ({
    ...
    const onSubmit = async (data: ProvinceFormValues) => {
        try {
            setLoading(true);
            if (initialData) { 
                await axios.patch(`/api/${params.storeId}/provinces/${params.provinceId}`, data)
            } else {
                await axios.post(`/api/${params.storeId}/provinces`, data)
            }
            router.refresh()
            router.push(`/${params.storeId}/provinces`)
    ...
    const onDelete = async () => {
        try {
            setLoading(true);
            await axios.delete(`/api/${params.storeId}/provinces/${params.provinceId}`)
            router.refresh()
            router.push(`/${params.storeId}/provinces`)
            toast.success("province deleted.")
        } catch (error) {
            toast.error("Make sure you removed all products using this province first.")
        } finally {
            setLoading(false)
            setOpen(false)
        }
                        ...
                        <FormField 
                            ...
                            render={({field}) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input disabled={loading} placeholder="Province name" {...field}/>
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                            ...
                            <FormField 
                                ...
                                render={({field}) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <Input disabled={loading} placeholder="Province value" {...field}/>
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
